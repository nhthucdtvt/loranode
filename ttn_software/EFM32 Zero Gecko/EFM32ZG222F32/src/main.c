...
void send_string(USART_TypeDef *usart, LEUART_TypeDef *leuart, char * string)
{
      while (*string != 0)
      {
    	  if (usart != 0) {
				 USART_Tx(usart, *string++);
    	  } else if (leuart != 0) {
...
void LEUART0_IRQHandler()
{
	if (LEUART0->IF & LEUART_IF_RXDATAV)
	  {
		char test_char = LEUART_Rx(LEUART0);
		//LEUART_Tx(LEUART0, (uint8_t)test_char);
		USART_Tx(USART1, (uint8_t)test_char);
	  }
}
...
int main(void)
{
  ...
  send_string(USART1, 0, PMTK_SET_NMEA_OUTPUT_RMCONLY);
  /* Infinite loop */
  while (1) {

	  if(GPIO_PinInGet(BT0_PORT,BT0_PIN) == 0) {
		  GPIO_PinOutSet(LED0_PORT,LED0_PIN);
		  Delay(500);
		  GPIO_PinOutClear(LED0_PORT,LED0_PIN);

		  ftoa(floatToString,Lat);
		  sprintf(dataToString,"Lat: %s\n",floatToString);
		  send_string(0, LEUART0, dataToString);
		  ftoa(floatToString,Lng);
		  sprintf(dataToString,"Lng: %s\n", floatToString);
		  send_string(0, LEUART0, dataToString);

		  sprintf(dataToString,"Date:%u/%u/%u\n", date, month, year + 2000 );
		  send_string(0, LEUART0, dataToString);
		  sprintf(dataToString,"Time:%u:%u:%u\n", hour, minute, sec);
		  send_string(0, LEUART0, dataToString);
	  }
	  if(GPIO_PinInGet(BT1_PORT,BT1_PIN) == 0) {
		  GPIO_PinOutSet(LED1_PORT,LED1_PIN);
		  Delay(500);
		  GPIO_PinOutClear(LED1_PORT,LED1_PIN);
		  send_string(0, LEUART0, "test String void\r\n");
	  }
  }
}
